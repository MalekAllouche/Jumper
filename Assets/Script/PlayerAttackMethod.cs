...
namespace EthanTheHero
{
	public class PlayerAttackMethod : MonoBehaviour
	{
		...
		private readonly string attackAnimationName = "Attack";
		#region FIELD
        ...
        public void AttackP()
        {
            ...
            if (colInfo != null)
            {
                ITakeDamage damageable = colInfo.GetComponent<ITakeDamage>();
                if (damageable != null)
                {
		...
		{
			Debug.Log("Attacking");
			if (playerAnimator.GetCurrentAnimatorStateInfo(0).IsName(attackAnimationName))
			{
				return true;
			}
			return false;
