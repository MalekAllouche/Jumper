...
namespace EthanTheHero
{
	public class PlayerMovement : MonoBehaviour
	{
		...
		void FixedUpdate()
		{
			...
			// Combine groundLayer and wallLayer to detect both as ground
			LayerMask combinedLayers = groundLayer | wallLayer;

			// Check if set box overlaps with ground or wall
			Collider2D collider = Physics2D.OverlapBox(groundCheckPoint.position, groundCheckSize, 0, combinedLayers);
			if (collider != null)
			{
				lastOnGroundTime = 0.1f;
				grounded = true;

				// Reset JumpCount when overlapping with ground or wall
				JumpCount = 0;
			}
			else
			{
		...
		private void jump()
		{
			// Check if jump limit has not been reached
			if (JumpCount < 1)
			{
			...
			if (grounded) // on player groundcheck collide reset
			{
				isJumping = false;
				JumpCount = 0;
			}
		...
		private void WallSlidngMechanic()
		{
			...
			if (wall)
			{
				...
				// Reset JumpCount when starting to slide on a wall
				JumpCount = 0;
			}
